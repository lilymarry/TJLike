...
#import "ShareSDK.h"
@interface FuLiDetailViewController ()<UITableViewDataSource,UITableViewDelegate,CommonDelegate>
{
...
@implementation FuLiDetailViewController
...
-(void)getFULiCommetData
{
    ...
    [AlertHelper MBHUDShow:@"加载中..." ForView:self.view AndDelayHid:30];
    [[WebApi sharedRequest]GetFuliCommetListWithId:_fuliId status:@"0" page:pageStr  Success:^(NSDictionary *dic){
        //   NSLog(@"SSSSS___ %@",dic);
        [AlertHelper hideAllHUDsForView:self.view];
        if ([[dic objectForKey:@"code"] integerValue] == 200) {
            NSArray *arr=dic[@"data"];
...
-(void)getFULiCanYUData
{
    ...
    //参与
    [AlertHelper MBHUDShow:@"加载中..." ForView:self.view AndDelayHid:30];
    [[WebApi sharedRequest]GetFuliCommetListWithId:_fuliId status:@"1"
                                              page:pageStr Success:^(NSDictionary *dic)
     {
         //   NSLog(@"SSSSS___ %@",dic);
         [AlertHelper hideAllHUDsForView:self.view];
         if ([[dic objectForKey:@"code"] integerValue] == 200) {
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [self.naviController setNaviBarTitle:@"福利详情"];
...
- (void)RightBtnClick{
  ...
  //  NSString *imagePath = [[NSBundle mainBundle] pathForResource:@"ShareSDK" ofType:@"png"];
        ...
        if (infoDic[@"focus"]) {
            image = [ShareSDK imageWithUrl:infoDic[@"focus"]];
         }
...
- (void)ShowCommentView:(UIButton *)sender{
    ...
    if ([UserAuthorManager gainCurrentStateForUserLoginAndBind]==0){
        ...
        [[WebApi  sharedRequest]AddUserCollectionWithUserId:UserManager.userInfor.userId  aid:_fuliId type:fuli_KEY Success:^(NSDictionary *dic){
            [AlertHelper hideAllHUDsForView:self.view];
            
            if ([[dic objectForKey:@"code"] integerValue] == 200) {
                [AlertHelper singleMBHUDShow:@"收藏成功" ForView:self.view AndDelayHid:3];
            }
            else if ([[dic objectForKey:@"code"] integerValue] == 502){
                [AlertHelper singleMBHUDShow:@"已存在" ForView:self.view AndDelayHid:3];
...
{  //  NSLog(@"3");
    ...
    if ([UserAuthorManager gainCurrentStateForUserLoginAndBind]==0){
        ...
        [AlertHelper MBHUDShow:@"提交中..." ForView:self.view AndDelayHid:30];
        [[WebApi  sharedRequest] AddHFCommentWithId:UserManager.userInfor.userId type:@"1" status:@"1" aid:_fuliId reply:@"0" font:@"我要参与" imgs:nil Success:^(NSDictionary *dic){
            [AlertHelper hideAllHUDsForView:self.view];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        FuliTittleViewCell *cell = (FuliTittleViewCell *)[tableView dequeueReusableCellWithIdentifier:CustomCellIdentifier];
        if (!cell) {
            NSArray* nibView =  [[NSBundle mainBundle] loadNibNamed:@"FuliTittleViewCell" owner:nil options:nil];
            cell= [nibView lastObject];
        }
   
        [cell.ima_Focus  sd_setImageWithURL:[NSURL URLWithString:infoDic[@"focus"]]                                placeholderImage:[UIImage imageNamed:@"news_list_default"]
                                  options:0
        ...
        cell.lab_Count.text=[NSString stringWithFormat:@"共 %@份",infoDic[@"count"]];
        cell.lab_Category.text=[NSString stringWithFormat:@"%@",infoDic[@"category"]];
        cell.lab_Supplier.text=[NSString stringWithFormat:@"%@",infoDic[@"supplier"]];
        cell.lab_Price.text=[NSString stringWithFormat:@"%@元",infoDic[@"price"]];
        return cell;
    }
        ...
        
    {
        ...
        NSString *stime=[NSString stringWithFormat:@"%@",  infoDic[@"startime"] ];
        NSString *etime=[NSString stringWithFormat:@"%@",  infoDic[@"endtime"] ];


        cell.lab_content.text=[NSString stringWithFormat:@"%@至%@",[self dateFromDate: stime ],[self dateFromDate: etime ] ];
        ...
        
    {
        ...
        cell.lab_tittle.text=[NSString stringWithFormat:@"%@",infoDic[@"get_idea"]];
