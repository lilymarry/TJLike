...
- (void)viewDidLoad {
    ...
    //UIView *bView = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(topView.frame)+2, SCREEN_WIDTH, 35)];
   // bView.backgroundColor = [UIColor clearColor];
   // bView.userInteractionEnabled = YES;
  //  [self.view addSubview:bView];
    
    UILabel *mLa= [[UILabel alloc]initWithFrame:CGRectMake(6,  CGRectGetMaxY(topView.frame)+2, SCREEN_WIDTH-100,20)];
    //mLa.numberOfLines = 2;
    mLa.backgroundColor = [UIColor clearColor];
    mLa.textColor = UIColorFromRGB(0x8a8a8a);
    mLa.font = [UIFont systemFontOfSize:14];
    mLa.text = @"请输入您的爱车车型和车牌号";
    mLa.textAlignment=NSTextAlignmentLeft;
    [self.view addSubview:mLa];
    
    NSArray *array = @[@"车型",@"车牌号  津*",@"备注"];
    NSArray *imaArr = @[@"carxing.png",@"carno.png",@"carbeizhu.png"];

    for (int i = 0; i<3; i++) {
        UIView *mView = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(mLa.frame)+2+i*45, SCREEN_WIDTH, 45)];
        ...
        UIImageView *icon = [[UIImageView alloc]initWithFrame:CGRectMake(10,12, 25, 25)];
        icon.image = [UIImage imageNamed:imaArr[i]];
        ...
        UILabel *mLabel = [[UILabel alloc]initWithFrame:CGRectMake(45, 2, 120, 45)];
        mLabel.backgroundColor = [UIColor clearColor];
        mLabel.textColor = [UIColor blackColor];
        mLabel.font = [UIFont systemFontOfSize:16];
        if (i==1) {
            NSMutableAttributedString *attrString = [[NSMutableAttributedString alloc] initWithString:array[i]];
            
            
            UIColor *color = [UIColor redColor];
            [attrString addAttribute:NSForegroundColorAttributeName
                               value:color
                               range:[array[i] rangeOfString:@"津"]];
             [mLabel setAttributedText:attrString];
        }
        else
        {
              mLabel.text = array[i];
        ...
        if (i==0) {
            UIImageView *icon1 = [[UIImageView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH-40, 13,10, 20)];
            icon1.image = [UIImage imageNamed:@"return.png"];
            [mView addSubview:icon1];
            
            
            mLabel1 = [[UILabel alloc]initWithFrame:CGRectMake(100,1, SCREEN_WIDTH-120, 45)];
            mLabel1.textColor = [UIColor blackColor];
            mLabel1.font = [UIFont systemFontOfSize:16];
            ...
            mField = [[UITextField alloc]initWithFrame:CGRectMake(140,1, SCREEN_WIDTH-150, 45)];
            ...
            mField2 = [[UITextField alloc]initWithFrame:CGRectMake(100, 2, SCREEN_WIDTH-120, 45)];
...
- (void)OnRegisterClick{
    ...
    //CarManager.carInfoArray
    TJFoundCarModel *model = [[TJFoundCarModel alloc]init];
    model.name = mField2.text;
    model.num = mField.text;
    model.model = mLabel1.text;
...
//        [dict setObject:mField2.text forKey:@"name"];//车名字
//        [dict setObject:mField.text forKey:@"num"];//车牌号
//        [dict setObject:@"1" forKey:@"limit"];//限行提醒 0不提醒
//        [dict setObject:@"0" forKey:@"check"];//年检提醒 0不提醒
//        [dict setObject:@"0" forKey:@"baoxian"];//保险  0不提醒
//        [dict setObject:@"0" forKey:@"jiazhao"];//驾照  0不提醒
